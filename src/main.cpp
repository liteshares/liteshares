...
uint256 hashGenesisBlock("0x2943ebd7cd79ebe8372092cee6fa6dd7feaa2f74a3fcacdee2784c755a4e8e07");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // LiteShares: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "LiteShares Signed Message:\n";
...
{
    // LiteShares: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    { nSubsidy = 25 * COIN;}
    else if (nHeight < 16001)
    { nSubsidy = 12.5 * COIN;}
    else if (nHeight < 32001)
    {nSubsidy = 6.25 * COIN;}
    else if (nHeight < 64001)
    {nSubsidy = 3.125 * COIN;}
    else if (nHeight < 128001)
    {nSubsidy = 1.5625 * COIN;}
    else if (nHeight < 72280001)
    {nSubsidy = 1 * COIN;}
    else
    {nSubsidy = 0 ;}
    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
   // nSubsidy >>= (nHeight / 840000); // LiteShares: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 20 * 60; // LiteShares: 3.5 days
static const int64 nTargetSpacing = 4 * 60; // LiteShares: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // LiteShares: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // LiteShares: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // LiteShares: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x2943ebd7cd79ebe8372092cee6fa6dd7feaa2f74a3fcacdee2784c755a4e8e07");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Today,The LiteShares is building to world for now...";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1390911667;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086227835;
        ...
        assert(block.hashMerkleRoot == uint256("0x8e3928ce687a5dbb6e3d6e82628a07e5e1c47d331085c5bc54d220aaa0fb1354"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // LiteShares: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LiteSharesMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LiteSharesMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LiteSharesMiner(CWallet *pwallet)
{
    printf("LiteSharesMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("LiteShares-miner");
    ...
    try { loop {
        ...
        printf("Running LiteSharesMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LiteSharesMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LiteSharesMiner, pwallet));
}
